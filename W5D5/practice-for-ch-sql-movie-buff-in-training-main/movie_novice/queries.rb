...
def below_average_years
  ...
  Movie.select('Count(yr) AS bad_movies', :yr).where('score < 5').group('yr').order('bad_movies DESC')
end
...
def alphabetized_actors
  ...
  Actor.select("actors.id, actors.name").order('name ASC').offset(10).limit(10)
end
...
def pulp_fiction_actors
  ...
  Actor.select(:id, :name).joins(:movies).where("movies.title LIKE 'Pulp Fiction'")
end
...
def uma_movies
  ...
  Movie.select('movies.id, movies.title, movies.yr').joins(:actors).where("actors.name LIKE 'Uma Thurman'").order('yr ASC')
end